...
The variables 'Issue date' en 'source' are not necessary for further analysis and will be removed from the dataset.
...
Based on the tests performed, it can be said that within all the variables studied, groups differ (at least one from another) among themselves in the context of rating.
...
The 'skewed' distribution of the wine scores raises some question marks. Are the wines that Robert Parker reviews really all good? Or does it still play a role that a specific colour, type or grape variety is generally found to be tastier?
...
Is the data representative enough for further research? In further research, the aforementioned factors (for example soil type) will be compared to these data.
